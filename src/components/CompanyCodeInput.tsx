...
const CompanyCodeInput = ({ onCodeVerified }: CompanyCodeInputProps) => {
    ...
    <Card className="border-dashed border-2 border-muted-foreground/30">
      <CardContent className="p-4 space-y-4">
        <div className="text-center">
          <h3 className="font-semibold font-armenian mb-2">Ընկերային կոդ</h3>
          <p className="text-sm text-muted-foreground font-armenian">
            Մուտքագրեք ձեր ընկերության կոդը՝ անվճար մուտք ստանալու համար
          </p>
        </div>

        <div className="space-y-3">
          <div>
            <Label htmlFor="company-code" className="font-armenian">Ընկերության կոդ</Label>
            <Input
              id="company-code"
              value={code}
              ...
              className="uppercase"
              disabled={isChecking || verificationStatus === 'valid'}
            />
          </div>

          <Button 
            onClick={handleVerifyCode}
            disabled={!code.trim() || isChecking || verificationStatus === 'valid'}
