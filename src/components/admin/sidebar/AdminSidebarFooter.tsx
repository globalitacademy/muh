...
const AdminSidebarFooter = ({ isCollapsed }: AdminSidebarFooterProps) => {
  ...
  return (
    <SidebarFooter className="border-t border-border/20 p-2 sm:p-3 bg-gradient-to-r from-muted/20 via-muted/40 to-muted/20 backdrop-blur-sm relative overflow-hidden">
      ...
      <div className="flex items-center justify-center relative z-10">
        <Button
          ...
          className={cn(
            ...
            isCollapsed ? [
              ...
              "w-full px-2 sm:px-4 py-1.5 sm:py-2 gap-1.5 sm:gap-2",
        ...
        >
          ...
          {isCollapsed ? (
            <PanelRight className="h-3 w-3 sm:h-4 sm:w-4 text-edu-blue transition-all duration-200 group-hover:scale-110 relative z-10" />
          ) : (
            <>
              <PanelLeft className="h-3 w-3 sm:h-4 sm:w-4 text-muted-foreground transition-all duration-200 group-hover:scale-110 group-hover:text-foreground relative z-10" />
              <span className="text-xs sm:text-sm font-armenian text-muted-foreground group-hover:text-foreground transition-colors duration-200 relative z-10">
                Փակել մենյուն
              </span>
            </>
          )}
