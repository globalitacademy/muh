...
const AdminSidebar = ({
  ...
  return <Sidebar variant="inset" className="border-r border-border/30 backdrop-blur-sm bg-card/50 transition-all duration-150 ease-out" collapsible="icon">
          ...
          <SidebarGroupContent>
            <SidebarMenu className="space-y-1">
              ...
              return <React.Fragment key={item.id}>
                    <SidebarMenuItem className="group/menu-item" style={{
                  animationDelay: `${index * 40}ms`,
                  animation: 'slideInLeft 0.3s ease-out forwards'
                }}>
                      <SidebarMenuButton onClick={() => handleSectionClick(item.id)} isActive={activeSection === item.id} tooltip={item.label} className={cn("font-armenian h-12 px-3 rounded-xl transition-all duration-200 group relative overflow-hidden", "hover:bg-sidebar-accent/80 hover:shadow-sm hover:scale-[1.01]", "focus-visible:ring-2 focus-visible:ring-edu-blue/50 focus-visible:outline-none", activeSection === item.id && ["bg-edu-blue/10 border border-edu-blue/20 shadow-sm", "text-edu-blue font-medium"])}>
                        <item.icon className={cn("w-5 h-5 transition-all duration-200 group-hover:scale-105", activeSection === item.id ? "text-edu-blue" : "text-muted-foreground group-hover:text-foreground")} />
                        <span className={cn("transition-all duration-200 group-data-[collapsible=icon]:sr-only flex-1", activeSection === item.id ? "text-edu-blue font-medium" : "text-foreground/80 group-hover:text-foreground")}>
                          {item.label}
                        </span>
                        
                        {/* Badge for notifications */}
                        {item.badge && <Badge variant={item.badge === 'Նոր' ? 'default' : 'secondary'} className={cn("ml-auto text-xs px-2 py-0.5 transition-all duration-200", "group-data-[collapsible=icon]:absolute group-data-[collapsible=icon]:top-1 group-data-[collapsible=icon]:right-1", "group-data-[collapsible=icon]:w-2 group-data-[collapsible=icon]:h-2 group-data-[collapsible=icon]:p-0 group-data-[collapsible=icon]:rounded-full", item.badge === 'Նոր' && "bg-green-500 text-white border-0", item.badge !== 'Նոր' && "bg-edu-blue text-white border-0")}>
