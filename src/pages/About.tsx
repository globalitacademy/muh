...
const About = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: ratingData
      } = await supabase.from('modules').select('rating').not('rating', 'is', null).eq('is_active', true);
      const averageRating = ratingData?.length > 0 ? ratingData.reduce((sum, module) => sum + (module.rating || 0), 0) / ratingData.length : 0;
      return {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data
      } = await supabase.from('profiles').select('*').eq('role', 'instructor').limit(6);
      return data || [];
    }
  });
  const values = [{
          ...
          <div className="container mx-auto px-4 relative">
            <div className="text-center mb-16">
              ...
              <p className="text-xl text-muted-foreground max-w-4xl mx-auto font-armenian leading-relaxed">
                {language === 'hy' ? 'Մենք նվիրված ենք ապագայի կրթության ստեղծմանը՝ միավորելով ժամանակակից տեխնոլոգիաները և մարդկային փորձառությունը' : language === 'ru' ? 'Мы посвящены созданию образования будущего, объединяя современные технологии и человеческий опыт' : 'We are dedicated to creating the education of the future by combining modern technology and human experience'}
              </p>
            </div>

            {/* Stats */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 mb-16">
              {achievements.map((stat, index) => <div key={index} className="modern-card glass-card rounded-2xl p-6 text-center group hover:scale-105 transition-all duration-300">
          ...
          <div className="container mx-auto px-4">
            <div className="text-center mb-16">
              ...
              <p className="text-xl text-muted-foreground max-w-3xl mx-auto font-armenian">
                {language === 'hy' ? 'Այն սկզբունքները, որոնք ղեկավարում են մեր աշխատանքը' : 'The principles that guide our work'}
              </p>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
              {values.map((value, index) => <Card key={index} className="modern-card course-card-hover border-0 shadow-lg group overflow-hidden h-full">
              ...
              <div className="glass-card rounded-2xl p-8 md:p-12">
                <div className="space-y-6">
                  <p className="text-lg leading-relaxed font-armenian">
                    {language === 'hy' ? '"Կրթություն առանց սահմանների" հարթակը ծնվել է 2023 թվականին՝ մի խումբ փորձառու մասնագետների կողմից, ովքեր հավատում էին, որ որակյալ կրթությունը պետք է լինի հասանելի բոլորի համար։' : language === 'ru' ? 'Платформа "Образование без границ" была создана в 2023 году группой опытных специалистов, которые верили, что качественное образование должно быть доступно каждому.' : 'The "Education Without Limits" platform was born in 2023 by a group of experienced professionals who believed that quality education should be accessible to everyone.'}
                  </p>
                  <p className="text-lg leading-relaxed font-armenian">
                    {language === 'hy' ? 'Մեր նպատակն է ստեղծել այնպիսի կրթական միջավայր, որտեղ ուսանողները կարող են զարգանալ և հասնել իրենց նպատակներին՝ անկախ իրենց գտնվելու վայրից կամ ֆինանսական հնարավորություններից։' : language === 'ru' ? 'Наша цель - создать образовательную среду, где студенты могут развиваться и достигать своих целей, независимо от местоположения или финансовых возможностей.' : 'Our goal is to create an educational environment where students can grow and achieve their goals, regardless of their location or financial capabilities.'}
                  </p>
                  <div className="flex items-center gap-3 text-edu-blue">
                    <CheckCircle className="w-5 h-5" />
                    <span className="font-armenian font-medium">
                      {language === 'hy' ? 'Գերազանց դասընթացներ ու մասնագիտական աջակցություն' : 'Excellent courses and professional support'}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <section className="py-20 bg-muted/20">
          <div className="container mx-auto px-4">
            <div className="text-center mb-16">
              ...
              <p className="text-xl text-muted-foreground max-w-3xl mx-auto font-armenian">
                {language === 'hy' ? 'Փորձառու մասնագետներ, որոնք նվիրված են ձեր հաջողությանը' : 'Experienced professionals dedicated to your success'}
              </p>
            </div>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 max-w-6xl mx-auto">
              {instructorsLoading ? Array.from({
              length: 6
            }).map((_, index) => <Card key={index} className="modern-card border-0 shadow-lg h-full">
                    <CardContent className="p-6 lg:p-8 text-center h-full flex flex-col">
                  ...
                  </Card>) : instructors && instructors.length > 0 ? instructors.map((instructor, index) => <Card key={instructor.id} className="modern-card course-card-hover border-0 shadow-lg group overflow-hidden h-full">
                    <CardContent className="p-6 lg:p-8 text-center h-full flex flex-col">
                      <div className="w-24 h-24 mx-auto mb-6 rounded-full overflow-hidden group-hover:scale-105 transition-transform duration-300 bg-gradient-to-br from-edu-blue to-purple-600 flex items-center justify-center">
                        {instructor.avatar_url ? <img src={instructor.avatar_url} alt={instructor.name || 'Դասախոս'} className="w-full h-full object-cover" /> : <Users className="w-12 h-12 text-white" />}
                      </div>
                      ...
                      <p className="text-muted-foreground text-sm font-armenian leading-relaxed flex-grow">
                        {instructor.department || instructor.field_of_study || (language === 'hy' ? 'Փորձառու մասնագետ' : 'Experienced professional')}
                      </p>
                    </CardContent>
