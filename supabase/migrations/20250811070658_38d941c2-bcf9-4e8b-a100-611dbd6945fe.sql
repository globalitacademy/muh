-- Fix the projects and profiles relationship issue first
-- Update useProjects hook to not use joins, just direct queries
...
CREATE TABLE IF NOT EXISTS project_tasks (
  ...
  assigned_to UUID, -- Remove foreign key reference to profiles for now
  created_by UUID NOT NULL, -- Remove foreign key reference to profiles for now
...
CREATE TABLE IF NOT EXISTS project_discussions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
  user_id UUID NOT NULL, -- Remove foreign key reference to profiles for now
  message TEXT NOT NULL,
  is_private BOOLEAN DEFAULT FALSE,
  recipient_id UUID, -- Remove foreign key reference to profiles for now
  files JSONB DEFAULT '[]'::jsonb,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS project_reviews (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id UUID NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
  participant_id UUID NOT NULL, -- Remove foreign key reference to profiles for now
  reviewer_id UUID NOT NULL, -- Remove foreign key reference to profiles for now
