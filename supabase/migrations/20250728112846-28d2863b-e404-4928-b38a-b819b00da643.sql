...
-- Create security definer function for user roles to avoid infinite recursion
...
SET search_path = public
AS $$
  SELECT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = user_uuid AND role IN ('admin', 'instructor'));
$$;
...
CREATE POLICY "Admins can manage specialties" ON public.specialties FOR ALL USING (public.is_admin_or_instructor(auth.uid()));

-- RLS Policies for modules
CREATE POLICY "Anyone can view active modules" ON public.modules FOR SELECT USING (is_active = true);
CREATE POLICY "Instructors can manage their modules" ON public.modules FOR ALL USING (public.is_admin_or_instructor(auth.uid()));
...
CREATE POLICY "Instructors can manage topics" ON public.topics FOR ALL USING (public.is_admin_or_instructor(auth.uid()));

-- RLS Policies for user_roles
CREATE POLICY "Users can view own roles" ON public.user_roles FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Instructors can manage roles" ON public.user_roles FOR ALL USING (public.is_admin_or_instructor(auth.uid()));
...
CREATE POLICY "Instructors can view all enrollments" ON public.enrollments FOR SELECT USING (public.is_admin_or_instructor(auth.uid()));
...
CREATE POLICY "Instructors can view all progress" ON public.user_progress FOR SELECT USING (public.is_admin_or_instructor(auth.uid()));

-- RLS Policies for exams
CREATE POLICY "Anyone can view active exams" ON public.exams FOR SELECT USING (is_active = true);
CREATE POLICY "Instructors can manage exams" ON public.exams FOR ALL USING (public.is_admin_or_instructor(auth.uid()));
...
CREATE POLICY "Instructors can view all exam registrations" ON public.exam_registrations FOR SELECT USING (public.is_admin_or_instructor(auth.uid()));

-- RLS Policies for announcements
CREATE POLICY "Anyone can view published announcements" ON public.announcements FOR SELECT USING (status = 'published');
CREATE POLICY "Instructors can manage announcements" ON public.announcements FOR ALL USING (public.is_admin_or_instructor(auth.uid()));
