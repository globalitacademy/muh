...
-- Policies
CREATE POLICY IF NOT EXISTS "Applications: applicant can create own" 
ON public.project_applications FOR INSERT 
WITH CHECK (applicant_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Applications: applicant can view own" 
ON public.project_applications FOR SELECT 
USING (applicant_id = auth.uid());

CREATE POLICY IF NOT EXISTS "Applications: creator or members can view" 
ON public.project_applications FOR SELECT 
USING (EXISTS (
...
CREATE POLICY IF NOT EXISTS "Applications: manage by creator or admin/instructor" 
...
CREATE POLICY IF NOT EXISTS "Applications: delete by creator or admin/instructor" 
